...
async function defaultLoadMessages(
  ...
  // Ownership is already checked by the lifecycle conversation query. RLS on
  // messages provides the second boundary here.
...
async function readRevisionMetadata(
  conversationId: string,
  ...
  for (const entry of entries) {
    ...
    const raw: unknown = JSON.parse(
      await readFile(`${directory}/${entry}`, 'utf8'),
    );
    ...
    revisions.push(raw as RevisionMetadata);
  }
...
async function ensureExistingRevisionMatches(
  ...
  if (codeSha256(build.code) !== metadata.codeSha256) {
    throw new Error(
...
async function persistBuildsLocked(
  ...
  const revisions = await readRevisionMetadata(conversationId);
  const byToolCall = new Map(
    revisions.map((metadata) => [metadata.toolCallId, metadata]),
  );
  let nextRevision =
    revisions.reduce((max, metadata) => Math.max(max, metadata.revision), 0) + 1;
...
async function withConversationLock<T>(
  ...
  conversationLocks.set(conversationId, previous.then(() => gate));
  ...
  try {
    ...
    release();
    if (conversationLocks.get(conversationId) === gate) {
