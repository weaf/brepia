...
VALUES 
  (uuid_generate_v4(), (SELECT id FROM auth.users WHERE email = 'test@brepia.invalid'), format('{"sub":"%s","email":"%s"}', (SELECT id FROM auth.users WHERE email = 'test@brepia.invalid')::text, 'test@brepia.invalid')::jsonb, 'email', uuid_generate_v4(), current_timestamp, current_timestamp, current_timestamp);
