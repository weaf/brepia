...
Use only schema-supported semantic selectors and project-object fields. Do not use filesystem, shell, network, native-kernel, Python/build123d, STEP, tessellation, viewer meshes, raw topology identifiers, or unrelated tools for this CAD task. Do not use `pcad_validate`; it is OpenSCAD-only. No Native BRep CAD tool is exposed through this external transport. Return only the structured final-result contract supplied by Brepia; Brepia validates that JSON result and performs the internal native build after the external response.
