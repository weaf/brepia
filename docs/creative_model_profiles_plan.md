...
Status: **Phase 1 implementation in progress on `feature/creative-model-profiles`**
...
- During Phase 1, the selected profile is materialized into the existing `nativeImageModelId` / `nativeMeshModelId` runtime slots so the native generation path does not change protocol or stable-runtime behavior.
...
- [ ] Resolve native runtime models from the pinned profile rather than only the current global active profile.
...
5. Local Creative still generates text → 3D and reference-image → 3D with the selected profile.
6. Hosted provider-specific routing is absent in a local-only deployment.
7. Stable-runtime behavior remains unchanged while generation is in progress.
