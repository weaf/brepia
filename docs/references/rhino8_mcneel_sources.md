...
For M4 profile extrusion, the official RhinoCommon `Rhino.Geometry.Extrusion.Create(Curve, Plane, double, bool)` API and `Extrusion.ToBrep()` are the direct SDK authority for the bounded host translation. The relevant `Extrusion` and explicit `Plane(origin, xDirection, yDirection)` surfaces are within the Rhino 8 compatibility floor. Brepia does not ask Rhino to infer the profile workplane: it constructs the canonical right-handed local frame explicitly, moves the plane origin to `-depth/2` along the extrusion normal, supplies the positive full depth to `Extrusion.Create(..., cap: true)`, converts the result to a Brep and requires one closed solid. Rectangle and circle profiles are centered in that plane; closed polylines use ordered local U/V coordinates and explicit curve closure. This is repository translation support only until fresh generated M4 GHX is opened, solved, parameter-perturbed, saved, closed and reopened in the installed Rhino 8 / Grasshopper host.

For the bounded planar circular sweep candidate, the reviewed branch-8 McNeel references are `rhinocommon/cs/SampleCsCommands/SampleCsSweep1.cs` and `rhinocommon/snippets/py/sweep-surfaces-with-sweep1.py`. The C# sample calls `Brep.CreateFromSweep(...)` with the active document absolute tolerance; the Python sample configures `SweepOneRail` with document angle/absolute tolerances and a roadlike frame. RhinoCommon 8 also exposes `Brep.CapPlanarHoles(tolerance)`. Brepia's first sweep slice keeps frame/twist settings out of canonical state because the section is circular; the compiler must still construct the canonical rail and start section explicitly, require exactly one closed solid Brep, and obtain installed Rhino 8 / Grasshopper evidence before runtime parity is claimed.
...
For M4 extrusion, keep profile data inline in the `extrude` node rather than introducing Rhino curve/profile identity into the canonical graph. Construct the exact canonical U/V frame explicitly: X extrusion uses U=Y and V=Z, Y extrusion uses U=Z and V=X, and Z extrusion uses U=X and V=Y. Start the Rhino extrusion plane at `-depth/2` along its normal and extrude the positive full depth so the resulting solid remains centered like the authoritative native implementation. Require a valid closed profile, a non-null `Extrusion`, and a solid Brep after `ToBrep()`. Kernel-neutral canonical validation remains responsible for bounded profile cardinality and simple-polyline validity before the host path runs. Installed-host evidence is required before M4 Rhino parity is considered accepted.
